...
command_args+=( --epoch-fee-numerator 5 )
command_args+=( --epoch-fee-denominator 10000 )

# Withdrawal fee for SOL and stake accounts, represented as `numerator / denominator`
command_args+=( --withdrawal-fee-numerator 5 )
command_args+=( --withdrawal-fee-denominator 10000 )
...
command_args+=( --max-validators 100 ) # Maximum number of validators in the stake pool, 2350 is the current maximum possible
...
# echo "Owner"
# # owner_keyfile=/Users/mgermain/.config/solana/devnet.json
# solana account $owner_keyfile
...
$spl_stake_pool \
  ...
  # --manager "$owner_keyfile" \
  # --staker "$owner_keyfile" \
  # --fee-payer "$owner_keyfile" \
...
$spl_stake_pool \
  ...
  "$stake_pool_pubkey" \
  "Praying Otter Solana Pool" \
  "otterSOL" \
  https://raw.githubusercontent.com/kilnmg/spl-solana-pool/main/metadata.json
